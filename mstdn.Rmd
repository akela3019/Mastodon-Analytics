...
newtheme <- theme(
  ...
  plot.title = element_text(size = 10, face = "plain", color = col_text),
...
update_geom_defaults("text", list(family = frank_med, size = 3, color = col_text))
update_geom_defaults("richtext", list(family = frank_med, size = 3, color = col_text))
update_geom_defaults("text_repel", list(family = frank_med, size = 3, color = col_text))
...
nodetext <- network_df %>%
  ...
  arrange(name)%>%
  slice(round(n()/2))%>%
  cbind(label = c("WSC", "Chess 03", "TGC Book", "COVID-19", "GQ",
                   "CoH", "snow", "MMAL", "Gondolin", "L5Y", "Ars\nNova"))%>%
  mutate(label = paste0(label, " (", n_toot, ")"))

p_network <- network_df %>%
  ...
  geom_nodetext(data = nodetext,
                ...
                nudge_x = c(-0.25, 0, -0.01, 0, -0.17, 
                            -0.02, -0.18, 0.15, 0.48, 0.09, -0.15),
                nudge_y = c(-0.05, 0.15, -0.24, -0.06, -0.19,
                            0.05, 0.03, -0.14, 0.07, 0.07, -0.03))+
  labs(title = "Toot Threads")+
  scale_color_manual(values = c('#30a7fc', pal[-1], 'salmon2'))+
  newtheme_void+
...
p_date <- date_df %>%
  ...
  geom_text(data = subset(date_df, n == max(date_df$n))%>%
              mutate(label = paste0(n, " (" , month(date, label = TRUE), 
                                    " ", day(date), ")")),
            aes(y = n + 1, label = label), size = 2.4,
            hjust = 0.08)+
...
p_emoji <- emoji_hashtag_df %>%
  ...
  geom_text(aes(x = n + c(-3, rep(1, 4)), label = paste0(n, " (", prop, "%)"), 
                hjust = c(1, rep(0, 4))),
...
g <- arrangeGrob(p_date + theme(plot.margin = margin(5, 5, 40, 7),
            ...
            p_hashtag + theme(plot.margin = margin(-45, 5, 25, 5),
                              plot.title = element_text(size = 9)),
            p_network + theme(plot.margin = margin(-25, -3, -5, -15),
                              plot.title = element_text(hjust = 0.1)),
            p_sondheim$p1 + theme(plot.margin = margin(0, 10, 27, 5)),
            p_tolkien$p1 + theme(plot.margin = margin(-15, 10, 42, 5)),
            p_malloy$p1 + theme(plot.margin = margin(-30, 10, 57, 5)),
            p_sondheim$p2 + theme(plot.margin = margin(-5, -5, 25, -5)),
...
g
#ggsave("mstdn_stat.png", g, width = 7, height = 7.5, dpi = 400)
```
